  ...
  fetch('/quizzy-api-quiz/' + id)  
      ...
      .then((data) => {
        ...
        <div class="image-container">
            <img src="${element.image}" class="quiz-image" alt="...">
        </div>
        <div class="answers-container">
            <a class="answer-btn" role="button" onclick="checkAnswer(${ element.id }, '${JSON.parse(element.answers)[0].text}', ${ element.quiz_id })"><span>${JSON.parse(element.answers)[0]['text']}</span></a>
            <a class="answer-btn" role="button"  onclick="checkAnswer(${ element.id }, '${JSON.parse(element.answers)[1].text}', ${ element.quiz_id })"><span>${JSON.parse(element.answers)[1].text}</span></a>
        </div>
        <div class="answers-container">
          <a class="answer-btn" role="button"  onclick="checkAnswer(${ element.id }, '${JSON.parse(element.answers)[2].text}', ${ element.quiz_id })"><span>${JSON.parse(element.answers)[2].text}</span></a>
          <a class="answer-btn" role="button"  onclick="checkAnswer(${ element.id }, '${JSON.parse(element.answers)[3].text}', ${ element.quiz_id })"><span>${JSON.parse(element.answers)[3].text}</span></a>
        </div>
        <div>
          <p class="question-count">Question: ${counter+1}/${data.length}</p>
      </div>
      `
        ;
      }
